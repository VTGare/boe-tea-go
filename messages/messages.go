...
func LimitExceeded(limit, artworks, count int) string {
	if artworks == 1 {
		return fmt.Sprintf("Album size `(%v)` exceeds the server's limit `(%v)`, album has been cut.", count, limit)
	}

	return fmt.Sprintf("Album size `(%v)` exceeds the server's limit `(%v)`, only the first image of every artwork has been sent.", count, limit)
}
...
func RateLimit(duration time.Duration) string {
	return fmt.Sprintf("Calm down, you're getting rate limited. Try again in **%v**", duration.Round(1*time.Second).String())
}

func NoPerms() string {
	return "~~Skill issue.~~ Not enough permissions to run this command."
}

func NSFWCommand(cmd string) string {
	return fmt.Sprintf("Bonk! You're trying to run an unsafe command `%v` in a safe channel.", cmd)
}
